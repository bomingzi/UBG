...
class EasyEnv(TPSEnv):
    ...
    def _compute_reward(self):
        ...
        gun_xywh = None
        enemy_xywh = None
        gun_reward = 15 * self.state["has gun"]
        reward = -3
        enemy_reward = 10 * self.state["target hit"]
        conf = 0
        pos_dist_reward = 0
        enemy_dist_reward = 0
        ...
        if self.state["stage"][0] == 0:
            # if self.state["detect gun"][0] == 1:
            #     gun_xywh = self.state["detect gun"][1:5].numpy()
            #     # conf = self.state["detect gun"][5]
            #     gun_reward += 5 * ((1 - (gun_xywh[1] / np.sqrt(1 / 2))) - 0.18) * conf
            #     # print("gun_reward: ", gun_reward)
            ...
            # #         conf = line[5].numpy()
            # #         break
            # if gun_xywh is not None:
            #     gun_dist = np.linalg.norm((0.4, 0.65) - np.array([gun_xywh[0], gun_xywh[1]]))
            #     # print("gun_dist: ", gun_dist)
            #     # gun_area = np.array(gun_xywh[2] * gun_xywh[3]) * 70
            #     # print("gun_area: ", gun_area)
            #     gun_reward += 5 * ((1 - (gun_dist / np.sqrt(1 / 2))) - 0.18)  # * gun_area * conf + 0.3
            #     # print("gun_reward: ", gun_reward)
            ...
            if self.state["detect enemy"][0] == 2 and self.prev_state["detect enemy"][0] == 2:
                enemy_xywh = self.state["detect enemy"][1:5].numpy()
                prev_enemy_xywh = self.prev_state["detect enemy"][1:5].numpy()
                ...
                enemy_dist_reward = ((prev_enemy_dist - enemy_dist) / 0.5)

            # for line in detect_info:
            #     if line[5] < 0.5:
            #         break
            #     if line[0] == 2:
            #         enemy_xywh = line[1:5].numpy()
            #         conf = line[5].numpy()
            #         break
            # if enemy_xywh is not None:
            #     self.detect_enemy = enemy_xywh
            #     enemy_dist = np.linalg.norm((0.5, 0.5) - np.array([enemy_xywh[0], enemy_xywh[1]]))
            #     # print("gun_dist: ", gun_dist)
            #     # gun_area = np.array(gun_xywh[2] * gun_xywh[3]) * 70
            #     # print("gun_area: ", gun_area)
            #     enemy_reward += (5 * (1 - (enemy_dist / 0.5)) + dist_reward)  # * gun_area * conf + 0.3
            #     # print("enemy_reward: ", enemy_reward)
            # elif self.detect_enemy is not None:
            #     enemy_dist = np.linalg.norm((0.5, 0.5) - np.array([self.detect_enemy[0], self.detect_enemy[1]]))
            #     enemy_reward += (5 * (1 - (enemy_dist / 0.5)) + dist_reward)
            #     # print('no detect, no kill')
            # else:
            #     enemy_reward += dist_reward
            #     # print('never detect, no kill')
        # print("gun_reward: ", gun_reward)
            ...
            if dist > 3000:
                ...
                pos_dist_reward = 5 * (1 - (dist / 2000)) + 3
            # enemy_reward += dist_reward
            if self.state["detect enemy"][0] == 2 and self.prev_state["detect enemy"][0] == 2:
                enemy_xywh = self.state["detect enemy"][1:5].numpy()
                prev_enemy_xywh = self.prev_state["detect enemy"][1:5].numpy()
            ...
            #     enemy_dist = np.linalg.norm((0.5, 0.5) - np.array([self.detect_enemy[0], self.detect_enemy[1]]))
            #     enemy_reward += (5 * (1 - (enemy_dist / 0.5)) + dist_reward)
            #     # print('no detect, kill 1')
            # else:
            #     enemy_reward += dist_reward
            #     # print('never detect, kill 1')
            ...
            pos_dist_reward = -beta * math.log((dist / 3800), 1.5) + 6
        ...
        # print(dist)
        reward += gun_reward + enemy_reward + pos_dist_reward - self.stay_cnt * 0.01 - self.game_cnt * 0.01
        # print("reward: ", reward)
    ...
    def reset(self):
        ...
        self.reward = 0
        self.rifle_cnt = 0
        self.prev_target_hit = 0
        self.state["game over"] = np.array([0])
        self.state["stage"] = np.array([0])
