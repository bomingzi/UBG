...
class EasyEnv(TPSEnv):
    ...
    def _get_obs(self):
        ...
        self.state["has gun"] = np.array(status[0])  # 1 mean has gun
        ...
        self.detect_info = np.zeros((1, 16))
        i = 0
    ...
    def detect(self, img0):
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                # detect_matrix[int(cls), int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])] = conf
    ...
    def _compute_reward(self):
        ...
        if self.state["detect enemy"][0][0] == 2 and self.prev_state["detect enemy"][0][0] == 2 and self.state["has gun"] == 1:
            ...
            enemy_dist_reward = 0.1*((prev_enemy_dist - enemy_dist) / 0.5) * 2  # kaolv (1-x1)^2 - (1-x2)^2
        if self.state["stage"][0] == 0:
            ...
            if self.state["position"][1] > 6000:
                done = 1
                pos_dist_reward = -1

            else:
                # dist = abs(self.state['position'][1] - 5000)/1300
                # pos_dist_reward = (1 - dist) * 0.3
                pos_dist_reward = 0
            ...
            exit_pt = [-2630, 7911]
            dist = np.linalg.norm(self.state["position"][0:2] - enemy_pt)
            angle = math.atan2(self.state["position"][1] - exit_pt[1], self.state["position"][0] - exit_pt[0]) * 180 / math.pi
            target_angle = angle - 180 if angle > 0 else angle + 180
            delta_angle = abs(self.state["position"][4] - target_angle) if target_angle*self.state["position"][4] > 0 else 360 - abs(self.state["position"][4] - target_angle)
            if delta_angle > 180:
                delta_angle = 360 - delta_angle
            if dist > 3000 or self.state["position"][0] < -900:
                done = 1
                pos_dist_reward = -1
            else:
        ...
        # print(dist)
        reward += gun_reward + enemy_reward + pos_dist_reward - self.stay_cnt * 0.003 - self.game_cnt * 0.001 - self.rifle_cnt * 0.001
        # print("reward: ", reward)
    ...
    def _do_action(self, action):
        ...
        if action == 0:  # stay
            ...
            reply = self.req.make_request(self.msg)
            time.sleep(0.05)
        elif action == 5:  # rotate right lightly
            # self.stay_cnt += 1
            self.msg.msg_content = cmd_rotate([0.7, 0]).to_bytes()
            reply = self.req.make_request(self.msg)
        elif action == 6:  # rotate left lightly
            # self.stay_cnt += 1
            self.msg.msg_content = cmd_rotate([-0.7, 0]).to_bytes()
            reply = self.req.make_request(self.msg)
        elif action == 7:  # rotate right
            self.stay_cnt += 1
            self.msg.msg_content = cmd_rotate([10, 0]).to_bytes()
            reply = self.req.make_request(self.msg)
        elif action == 8:  # rotate left
            self.stay_cnt += 1
            self.msg.msg_content = cmd_rotate([-10, 0]).to_bytes()
            reply = self.req.make_request(self.msg)
        elif action == 9:  # jump, Maintain inertia, full action take about 0.8s
            self.stay_cnt += 1
...
if __name__ == "__main__":
    ...
    test = EasyEnv(record=False, image_shape=(84, 168, 1), yolo_weight='../weights/best.pt')
    test.reset()
    # while True:
    #     # aaa = test.step(1)
