...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, lstm_hidden=None, action=None):
        if x.dim() == 3:
            x = x.permute(2, 0, 1)
        elif x.dim() == 4:
            x = x.permute(0, 3, 1, 2)
        ...
        hidden, lstm_hidden = self.lstm(hidden, lstm_hidden)
        logits = self.actor(hidden)
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(hidden), lstm_hidden
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, lstm_hidden = agent.get_action_and_value(next_obs, lstm_hidden)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, newlstm_hidden = agent.get_action_and_value(b_obs[mb_inds],
