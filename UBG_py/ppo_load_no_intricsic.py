...
class Agent(nn.Module):
    def __init__(self):
        ...
        #     # layer_init(nn.Linear(512, 64)),
        #     # nn.ReLU(),
        #     layer_init(nn.Linear(512, env.action_space.nvec[3]), std=0.01),
        # )
    ...
    def get_value(self, x, g_info, detect_m):
        if x.dim() == 4:
            x = x.permute(0, 3, 1, 2)
        ...
        x = torch.cat((x, g_info, detect_m), dim=1)
        x = self.linear(x)
    ...
    def get_action_and_value(self, x, g_info, detect_m, action=None):
        ...
        return action, probs1.log_prob(action1) + probs2.log_prob(action2) + probs3.log_prob(
            action3), probs1.entropy() + probs2.entropy() + probs3.entropy(), self.critic(hidden)
...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="Ma_Env_NoIntrinsic",
...
if __name__ == "__main__":
    ...
    load_model = True
    model_load_path = 'outputs/ppo_ma_no_intrinsic_model.pt'
    if load_model and os.path.exists(model_load_path):
        ...
        # agent.actor4.load_state_dict(checkpoint['actor4'])
    ...
    data_load_path = 'outputs/data_save/ppo_data.pt'

    data = torch.load(data_load_path)
    demo_obs = data['obs']
    demo_actions = data['actions']
    demo_game_info = data['game_info'].squeeze(1)
    demo_detect_map = data['detect_map'].squeeze(1)
    print('data loaded from {}'.format(data_load_path))
    demo_num = demo_obs.shape[0]
    demo_batch_size = demo_num
    demo_mini_batch_size = int(demo_batch_size // 8)
    ...
    model_save_path = 'outputs/ppo_ma_no_intrinsic_model.pt'
    ...
    for update in range(begin_update, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs, next_game_info, next_detect_map)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs, next_game_info, next_detect_map).reshape(1, -1)
        ...
        demo_s_inds = np.arange(demo_num)
        demo_inds = np.arange(demo_batch_size)
        clipfracs = []
        np.random.shuffle(demo_s_inds)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue= agent.get_action_and_value(b_obs[mb_inds], b_game_info[mb_inds], b_detect_map[mb_inds],
            ...
            #     _, newlogprob, entropy, newvalue = agent.get_action_and_value(demo_obs[mb_inds], demo_game_info[mb_inds],
            #                                                                            demo_detect_map[mb_inds], demo_actions.long()[mb_inds])
            #     demo_loss1 = -newlogprob
            #     demo_loss2 = -torch.clamp(newlogprob, -2.5, -0.5)
            #     demo_loss = 0.1 * 0.98**update * torch.max(demo_loss1, demo_loss2).mean()
            #
            #     optimizer.zero_grad()
            #     demo_loss.backward()
            #     nn.utils.clip_grad_norm_(agent.parameters(), args.max_grad_norm)
