...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, detect, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(hidden), find
...
def parse_args():
    ...
    parser.add_argument("--num-steps", type=int, default=12,#8 * 4,
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, find = agent.get_action_and_value(next_obs, next_detect)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, newfind = agent.get_action_and_value(b_obs[mb_inds], b_detects[mb_inds],
