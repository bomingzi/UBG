...
class Agent(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Sequential(
            layer_init(nn.Linear(16 + 512, 512), std=0.01),
            nn.ReLU(),
        )
        self.lstm = layer_init(nn.LSTM(512, 512, batch_first=True))

    def get_value(self, x, detect, hx, cx):
        if x.dim() == 4:
            x = x.permute(0, 3, 1, 2)
    ...
    def get_action_and_value(self, x, detect, action=None, hx=None, cx=None):
        if x.dim() == 4:
            x = x.permute(0, 3, 1, 2)
        elif x.dim() == 3:
            x = x.permute(2, 0, 1)
        ...
        hidden, (hx, cx) = self.lstm(hidden, (hx, cx))
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action), probs.entropy(), self.critic(hidden), hx, cx
...
if __name__ == "__main__":
    ...
    hxs = torch.zeros((args.num_steps, args.num_envs) + (512,)).to(device)
    cxs = torch.zeros((args.num_steps, args.num_envs) + (512,)).to(device)
    ...
    model_save_path = 'outputs/ppo_dict_model.pt'
    ...
    for update in range(1, num_updates + 1):
        ...
        hx = torch.zeros((args.num_envs, 512)).to(device)
        cx = torch.zeros((args.num_envs, 512)).to(device)
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, _, value, hx, cx = agent.get_action_and_value(next_obs, next_detect, hx=hx, cx=cx)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs, next_detect, hx, cx).reshape(1, -1)
        ...
        b_hxs = hxs.reshape((-1,) + (512,))
        b_cxs = cxs.reshape((-1,) + (512,))
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, newhx, newcx = agent.get_action_and_value(b_obs[mb_inds],
                                                                                            b_detects[mb_inds],
